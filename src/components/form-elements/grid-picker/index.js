...
const GridPicker = ({ type, requireDefault, defaultValue, data, onSelect }) => {
  ...
  const imageRenderer = () => {
    switch (type) {
      case "color":
        return (
          <span className="swatch" style={{ background: selection.hex }}></span>
        );
      case "font":
        return (
          <span className="font-text" style={{ fontFamily: selection.title }}>
            {" "}
            Aa{" "}
          </span>
        );
          ...
          onClick={() =>
            trigger(
              requireDefault && type === "color"
                ? data.find((color) => color.id === DEFAULT_THEME)
                : null
            )
          }
        >
