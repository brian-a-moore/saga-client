...
const Picker = ({ defaultTags, availableTags }) => {
    ...
    const addTag = useCallback(
        (tag) => {
            ...
            if (text.length) {
                const filteredAvailableTags = availableTags.reduce(
                    (acc, tag) => {
                        if (tag.title.toLowerCase() === text.toLowerCase())
                            acc.push(tag);
                        return acc;
                    },
                    []
                ...
                const findTag = () => {
                    return availableTags.find(
                        (tag) => tag.title.toLowerCase() === text.toLowerCase()
                    );
                };
    ...
    useEffect(() => {
        ...
        for (let x in selectedTags)
            titles.push(selectedTags[x].title.toLowerCase());
        const preFilteredTags = availableTags.filter(
            (tag) => !titles.includes(tag.title.toLowerCase())
        );
